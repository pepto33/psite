...
define('SECURITY_PHRASE', '9xcqZcGi1AytW04eo7fAIjwgqVHzwb');
define('PASSWORD', '');
...
define('HTML_LANGUAGE', 'ru-RU');

define('TEXT_GALLERY_NAME', 'Single File PHP Gallery');
define('TEXT_BANNER', '');
define('TEXT_HOME', 'Домой');
define('TEXT_CLOSE_IMG_VIEW', 'Закрыть изображение');
define('TEXT_ACTUAL_SIZE', 'Актуальный размер');

define('TEXT_PREVIOUS', '&#x25C4;&#xFE0E; Преведущий');
define('TEXT_NEXT', 'Следуюший &#x25BA;&#xFE0E;');
define('TEXT_INFO', 'Информация');
define('TEXT_DOWNLOAD', 'Скачать изображение');
define('TEXT_SLIDESHOW', 'Слайдшоу');
define('TEXT_NO_IMAGES', 'Изображений в директории нет');
...
{
	$tmp = explode('/', $input);
	if (count($tmp) == 2)  // info: if input contains one /, like: "1/250"
	{
		if (is_numeric($tmp[0]) and is_numeric($tmp[1]) and ($tmp[1] != 0)) //info: if both sides are numbers and $tmp[1] is not 0.
		{
			return $tmp[0] / $tmp[1];
		}
	}
	...
	if ($func == 'thumb') {
		if (file_exists($image_path_file)) {
			...
			if (($func == 'thumb') and ($image_dir != ICONS_DIR)) {
				...
				$image_info['fileMTime'] = filemtime(GALLERY_ROOT . $image_dir . $image_file); // info: also used for deleting data if time have changed
...
function sfpg_javascript()
{
	...
	foreach ($dirs as $val) {
		...
		if (DELETE_EMPTY_DIRS and ($filed['dirs'] == 0) and ($filed['images'] == 0) and ($filed['files'] == 0)) {
			sfpg_set_dir_info(GALLERY . $val . '/'); // info: refreshing informarion before possible delete
			$filed = unserialize(file_get_contents(DATA_ROOT . 'info/' . GALLERY . $val . '/_sfpg_dir'));
		}
		if (DELETE_EMPTY_DIRS and ($filed['dirs'] == 0) and ($filed['images'] == 0) and ($filed['files'] == 0)) {
	...
	foreach ($images as $val) {
		...
		if ($showImage) {
			...
			if (file_exists(DATA_ROOT . 'info/' . GALLERY . $val)) {
				...
				if (filemtime(GALLERY_ROOT . GALLERY . $val) != $image_info['fileMTime']) // info: If image time have changed, delete data to have new thumb created
				{
					sfpg_delete_from_data_root(GALLERY . $val);
					$image_info['firstView'] = 1;
				} else {
					if (in_array($val . DESC_EXT, $misc)) // info: if description file exists, add to array
					{
...
if (PAYPAL_ENABLED and $get_set and ($_GET['cmd'] == 'buy') and IMAGE != '') {
	...
	echo '<body onresize="updateSize()" onload="showGallery(' . (IMAGE_ID_IN_URL ? IMAGE_ID_IN_URL : 'false') . ')" class="sfpg">' .
		...
		'<div class="sye"><small><a href="http://sye.dk/sfpg/" target="_blank" alt="">Single File PHP Gallery</a></small></div>' .
		'</div>' .
